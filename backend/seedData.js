...
const seedDB = async () => {
  try {
    ...
    if (existingCars > 0) {
      console.log(`⚠️  ${existingCars} cars already exist. Skipping car seeding.`);
      console.log('   To re-seed cars, first run: npm run seed:clean');
    } else {
      // Clear existing data ONLY if no cars exist
    ...
    const existingAdmin = await User.findOne({ email: 'admin@carcustomizer.com' });
    if (existingAdmin) {
